...
public class E2Test extends TestBase {
    ...
    public void createUpdateDeleteProject(){
        ...
        for (int i = 0; i < 4; i++){
            body = new JSONObject();
            String email = "bastavasta"+new Date().getTime()+"@abelardo.com";
            emails[i] = email;
        ...
        for (int i = 0; i < 4; i++){
            Configuration.user = emails[i];
            Configuration.password = "12345";
            deleteUser("adelete");
            System.out.println("User deleted: "+Configuration.user);
        }

        for (int i = 0; i < 4; i++){
            body = new JSONObject();
            body.put("Content","12345");
            Configuration.user = emails[i];
            Configuration.password = "12345";
            //readAllProjectsWithDeletedUser("aget", "");
...
//        this.readAllProjects(get, body.getString("Content"));
    ...
    private Response readAllProjectsWithDeletedUser(String get, String pName){
        requestInfo.setUrl(Configuration.host + "/api/projects.json");
        response = FactoryRequest.make(get).send(requestInfo);
        response.then().statusCode(200)
                ;
